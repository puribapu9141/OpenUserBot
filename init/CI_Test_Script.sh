...
tg_senderror() {
    ...
    tg_sendinfo "<code>Build Throwing Error(s)</code>" \
        ...
        "Logs: https://github.com/puribapu9141/OpenUserBot"
...
lint() {
  ...
  git config --global user.email "pratikgoswami9141@gmail.com"
  git config --global user.name "puribapu9141"
...
RESULT=`yapf -d -r -p userbot`
  ...
  if [ ! -z "$RESULT" ]; then
      ...
      if [ "$PERMIT_LINT" == "0" ]; then
            ...
            git remote set-url origin https://${GH_USERNAME}:${GH_PERSONAL_TOKEN}@github.com/puribapu9141/OpenUserBot.git
